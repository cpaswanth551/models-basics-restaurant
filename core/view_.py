...
def email_user(email):
    print(f"Dear {email}, Thank you for your orders..")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                )  # this will lock model untill the transaction is completed.
                order = form.save()
                # import sys
                # sys.exit(1)
                order.product.number_in_stock -= order.number_of_items
                order.product.save()
            transaction.on_commit(partial(email_user, "hello@g.com"))
...
def index_contexttype(request):
    ...
    # creating new object
    rest = Rating.objects.first()

    comment = Comment.objects.create(text="this is a bad commment", content_object=rest)
    ...
    # reverse relations
    rest = Restaurant.objects.get(pk=3)
    comments = rest.comments.all()

    # create a new object in reverse relations
    rest = Restaurant.objects.get(pk=3)
    rest.comments.add(Comment.objects.create(text="who are you", content_object=rest))

    print(comments)

    # delete operation using reverse relation (genericrelation)
    rest = Restaurant.objects.get(pk=3)
    print(rest.comments.count())

    last_comment = rest.comments.last()

    rest.comments.remove(last_comment)
    print(rest.comments.count())

    # filter  using related query name
